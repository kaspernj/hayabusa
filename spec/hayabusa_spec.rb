...
describe "Hayabusa" do
  ...
  File.unlink(db_path) if File.exist?t??(db_path)
  ...
  it "should be able to get multiple pictures" do
    ...
    raise "Invalid from path: '#{img_from_path}'." unless File.exist?t??(img_from_path)
    ...
    begin
      ...
      File.unlink(img_to_path) if File.exist?t??(img_to_path)
    end
  end
